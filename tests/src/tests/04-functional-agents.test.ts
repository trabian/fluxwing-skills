...
describe('Category 4: Functional Testing - Agents', () => {
  ...
  describe('Test 4.3: fluxwing-validator Agent Scope', () => {
    const skipIfNoApiKey = process.env.ANTHROPIC_API_KEY ? test : test.skip;

    skipIfNoApiKey('should only validate project files', async () => {
      ...
      const agentPrompt = 'Analyze all components in ./fluxwing/';

      const output = await client.dispatchAgent('fluxwing-validator', agentPrompt);

      // Should show project file paths
      FluxwingAssertions.assertContains(
        output,
        './fluxwing/',
        'Should validate project workspace files'
      );

      // Should NOT validate plugin directory
      FluxwingAssertions.assertNotContains(
        output,
        /.claude\/plugins.*validated/,
        'Should not validate plugin directory files'
      );
    });

    skipIfNoApiKey('should skip bundled templates with clear message', async () => {
      const agentPrompt = 'Validate all fluxwing components';

      const output = await client.dispatchAgent('fluxwing-validator', agentPrompt);

      // Should mention bundled templates and that they're skipped
      ...
      const mentionsSkip = skipIndicators.some((pattern) => pattern.test(output));

      expect(mentionsSkip).toBe(true);
    });

    skipIfNoApiKey('should show clear file paths in validation report', async () => {
      const agentPrompt = 'Validate components and provide detailed report';

      const output = await client.dispatchAgent('fluxwing-validator', agentPrompt);

      // Should use project-relative paths
      expect(output).toMatch(/\.\/fluxwing\//);

      // Should include file count or list
      expect(output).toMatch(/\d+.*file|component/i);
    });
  });
});
